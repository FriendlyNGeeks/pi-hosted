...
echo "STEP 1 OF 4 | Attempting to STOP docker container by ID: $unifiID." && sudo docker stop $unifi_pid && echo "STEP 1 SUCCESS" || error "Failed to stop unifi!"
echo "STEP 2 OF 4 | Proceeding to REMOVE docker container by ID: $unifiID." && sudo docker rm $unifi_pid && echo "STEP 2 SUCCESS" || error "Failed to remove unifi container!"
echo "STEP 3 OF 4 | Commencing with UNTAGGING/REMOVING docker images by NAME: $unifiNAME." && sudo docker rmi $unifi_name && echo "STEP 3 SUCCESS" || error "Failed to remove/untag images from the container!"
echo "STEP 4 OF 4 | Running docker up CLI." && sudo docker run -d --network host --name=unifi --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v config:/var/lib/unifi -v log:/usr/lib/unifi/logs -v log2:/var/log/unifi -v run:/usr/lib/unifi/run -v run2:/run/unifi -v work:/usr/lib/unifi/work ryansch/unifi-rpi:latest && echo "STEP 4 SUCCESS" || error "Failed to execute newer version of Portainer!"
