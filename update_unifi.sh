...
echo "STEP 1 OF 4 | Attempting to STOP docker container by ID: $unifiID." && sudo docker stop $unifi_pid || error "Failed to stop unifi!"
echo "STEP 2 OF 4 | Proceeding to REMOVE docker container by ID: $unifiID." && sudo docker rm $unifi_pid || error "Failed to remove unifi container!"
echo "STEP 3 OF 4 | Commencing with UNTAGGING/REMOVING docker images by NAME: $unifiNAME." && sudo docker rmi $unifi_name || error "Failed to remove/untag images from the container!"
